...
Deno.serve(async (req) => {
  try {
    ...
    const { book, success_path = '/order-success', cancel_path = '/create', growth_session_id = '', growth_source = 'direct', growth_campaign = 'organic', growth_content_variant = 'default' } = body || {};
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      metadata: {
        ...
        growth_source: growth_source || 'direct',
        growth_campaign: growth_campaign || 'organic',
        growth_content_variant: growth_content_variant || 'default',
        relationship_category: book.relationship || '',
        book_tone: book.tone || '',
      },
    });
